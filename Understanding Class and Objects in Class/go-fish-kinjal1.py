...
def start_game():
    ...
    while player_change:
        ...
        while turn:
            # if total score == 13, 13 x 4 cards are found
            if sum(score.values()) == 13:
                player_change = False  # no more game
                print(score)
                break  # break the game and show the score
            else:
                # check if the current player has cards and there are cards on the deck
                ...
                # no cards with the current_player and no cards in the deck
                while not players[current_player] and not all_cards:
                    # end game if score == 13
                    if sum(score.values()) == 13:
                        player_change = False
                        print(score)
                        break
                ...
                # show only the current players cards
                if showing:
